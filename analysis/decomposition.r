...
path <- glue("output/{config$group}_measures_{config$set}{config$appt_suffix}{config$agg_suffix}/proc_{config$group}_measures_midpoint6")
...
for (measure_name in unique(data$measure)) {
  ...
  output_path <- glue("output/{config$group}_measures_{config$set}{config$appt_suffix}{config$agg_suffix}/decompositions/{measure_name}{ifelse(config$test, '_test', '')}.png")
  ...
  # Save model summary
  summary_path <- glue("output/{config$group}_measures_{config$set}{config$appt_suffix}{config$agg_suffix}/decompositions/summary_{measure_name}{ifelse(config$test, '_test', '')}.txt")
