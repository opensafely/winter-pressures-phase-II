...
choices = [True, False, True] # True for pre-pandemic, False for pandemic, True for post-pandemic
...
summer_df = (
    summer_df.groupby(['measure', 'summer_year'])['rate_per_1000_midpoint6_derived']
    .agg(['mean'])
    .rename(columns={'mean': 'summer_mean'})
    .reset_index()
)
...
# Save results
if args.test:
    results.to_csv('output/practice_measures/seasonality_results_test.csv')
else:
    results.to_csv('output/practice_measures/seasonality_results.csv')
