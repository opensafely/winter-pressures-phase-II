...
practice_weekly_df["interval_start"] = pd.to_datetime(
    practice_weekly_df["interval_start"]
)
...
print(
    column_total_check(
        practice_weekly_df, column="numerator", year=2023, measure_name="rsv_specific"
    )
)
...
practice_weekly_df["measure"] = practice_weekly_df["measure"].cat.set_categories(
    [DISEASE_TO_TEST]
)
...
national_weekly_df = build_aggregate_df(
    practice_weekly_df,
    ["measure", "interval_start"],
    {"numerator": "sum", "denominator": "sum", "practice_pseudo_id": "nunique"},
)

# Post-aggregation column edits
national_weekly_df.rename(
    columns={"practice_pseudo_id": "n_practices_week"}, inplace=True
)
national_weekly_df["rate_per_1000"] = (
    national_weekly_df["numerator"] / national_weekly_df["denominator"]
) * 1000

read_write(
    read_or_write="write",
    df=national_weekly_df,
    path=output_path,
    file_type="csv",
    test=False,
)
...
national_yearly_df.rename(
    columns={"practice_pseudo_id": "n_practices_year"}, inplace=True
)
national_yearly_df["rate_per_1000"] = (
    national_yearly_df["numerator"] / national_yearly_df["list_size_initial"]
) * 1000
national_yearly_df["year_start"] = CALENDAR_YEAR_TO_INCLUDE

read_write(
    read_or_write="write",
    df=national_yearly_df,
    path=f"{output_path}_yearly",
    file_type="csv",
    test=False,
)
